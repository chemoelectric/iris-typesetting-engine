            ...
            (let ((arg (car rest)))
              (cond
                ...
                ((and in-options? (string=? arg "--"))
                 (loop (cdr rest) #f apply-pegs? output-path format-override positional))

                ;; Short option -a (takes NO argument)
                ((and in-options? (string=? arg "-a"))
                 (loop (cdr rest) in-options? #t output-path format-override positional))
                ...
                ((and in-options? (string-prefix? "--apply=" arg))
                 (let ((val (string->boolean-value (substring arg 8 (string-length arg)))))
                   (loop (cdr rest) in-options? val output-path format-override positional)))

                ;; Output option: -o FILE, -oFILE, --output FILE, --output=FILE
                ((and in-options? (string=? arg "-o"))
                 (if (null? (cdr rest))
                     (loop (cdr rest) in-options? apply-pegs? output-path format-override positional)
                     (loop (cddr rest) in-options? apply-pegs? (cadr rest) format-override positional)))
                ((and in-options? (> (string-length arg) 2) (string-prefix? "-o" arg))
                 (loop (cdr rest) in-options? apply-pegs? (substring arg 2 (string-length arg)) format-override positional))
                ((and in-options? (string=? arg "--output"))
                 (if (null? (cdr rest))
                     (loop (cdr rest) in-options? apply-pegs? output-path format-override positional)
                     (loop (cddr rest) in-options? apply-pegs? (cadr rest) format-override positional)))
                ((and in-options? (string-prefix? "--output=" arg))
                 (loop (cdr rest) in-options? apply-pegs? (substring arg 9 (string-length arg)) format-override positional))

                ;; Format option: -f FORMAT, -fFORMAT, --format FORMAT, --format=FORMAT
                ((and in-options? (string=? arg "-f"))
                 (if (null? (cdr rest))
                     (loop (cdr rest) in-options? apply-pegs? output-path format-override positional)
                     (loop (cddr rest) in-options? apply-pegs? output-path (cadr rest) positional)))
                ((and in-options? (> (string-length arg) 2) (string-prefix? "-f" arg))
                 (loop (cdr rest) in-options? apply-pegs? output-path (substring arg 2 (string-length arg)) positional))
                ((and in-options? (string=? arg "--format"))
                 (if (null? (cdr rest))
                     (loop (cdr rest) in-options? apply-pegs? output-path format-override positional)
                     (loop (cddr rest) in-options? apply-pegs? output-path (cadr rest) positional)))
                ((and in-options? (string-prefix? "--format=" arg))
                 (loop (cdr rest) in-options? apply-pegs? output-path (substring arg 9 (string-length arg)) positional))

                ;; Combined or standalone flags starting with "-" (excluding "-" itself)
                ((and in-options? (> (string-length arg) 1) (char=? (string-ref arg 0) #\-))
                 (let ((has-a? (char-in-string? #\a (substring arg 1 (string-length arg)))))
                   (loop (cdr rest) in-options? (or apply-pegs? has-a?) output-path format-override positional)))

                ;; Positional argument
                (else
                 (loop (cdr rest) in-options? apply-pegs? output-path format-override (cons arg positional))))))))
