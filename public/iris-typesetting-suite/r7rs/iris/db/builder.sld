;;; r7rs/iris/db/builder.sld --- Database cache builder for Iris
;;;
;;; SPDX-License-Identifier: MIT

(define-library (iris db builder)
  (import (scheme base)
          (scheme char)
          (scheme file)
          (scheme write)
  ...
  (begin
    ...
    (define (ingest-texmf-file db dir fname)
      (let* ((full (string-append dir "/" fname))
        ...
        (db-set! db fname full)
        (unless (string=? norm fname)
          (db-set! db norm full))
        (db-set! db (string-append "name:" norm) fname)))
    ...
    (define (get-fc-list-entries)
      (guard (e (else '()))
        (let* ((fmt  (fc-list-format-string))
               (cmd  (list "fc-list" "-f" fmt))
               (raw  (process-output->string-list cmd))
               (rows (map string-trim-both raw)))
          (filter (lambda (s) (not (string=? s ""))) rows))))
    ...
    (define (get-part parts n dflt)
      (if (>= (length parts) n)
          (string-trim-both (list-ref parts (- n 1)))
          dflt))
    ...
    (define (index-single-alias db prefix alias id-str)
      (unless (string=? alias "")
        (let ((norm (normalize-name alias)))
          (db-set! db (string-append prefix alias) id-str)
          (db-set! db (string-append prefix norm) id-str)
          (db-set! db (string-append "name:" norm) alias))))

    (define (index-alias-list db prefix csv-str id-str)
      (let ((aliases (split-by-char csv-str #\,)))
        (for-each
         (lambda (a)
           (index-single-alias db prefix (string-trim-both a) id-str))
         aliases)))

    (define (index-font-metadata db id id-str path fam sty ps full)
      (let ((meta-str (make-font-record id path fam sty ps full)))
        (db-set! db (string-append "font:" id-str) meta-str)
        (db-set! db (string-append "file:" path) id-str)
