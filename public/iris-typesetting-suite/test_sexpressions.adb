...
procedure test_sexpressions is
   ...
   procedure assert_true (cond : in boolean; msg : in string) is
   begin
   ...
   procedure test_characters_and_strings is
      c : sexpr := make_character (sexpr_character'('a'));
   ...
   procedure test_pairs_and_lists is
      ...
      l1 : sexpr :=
        cons
          (make_integer (1),
           cons (make_integer (2), cons (make_integer (3), make_null)));
   begin
   ...
   begin
      ...
      assert_true (get_symbol (car (e1)) = "+", "car = +");
      assert_true (get_integer (cadr (e1)) = 10, "cadr = 10");
      assert_true (get_integer (list_ref (e1, 4)) = 31, "#x1f = 31");

      assert_true
        (is_boolean (e2) and then get_boolean (e2) = true, "read #t");
      assert_true
        (is_boolean (e3) and then get_boolean (e3) = false,
         "read #false");
      assert_true
        (is_character (e4) and then get_character (e4) = ' ',
         "read #\space");
      assert_true
        (is_character (e5)
         and then get_character (e5) = wide_wide_character'val (10),
         "read #\newline");
      assert_true
        (is_character (e10) and then get_character (e10) = 'a',
         "read #\a");
      assert_true
        (is_character (e11) and then get_character (e11) = 'b',
         "read #\x62;");
      assert_true
      ...
      assert_true
        (is_pair (e9) and then get_symbol (car (e9)) = "quote",
         "read quote");
   ...
   begin
      ...
      assert_true (get_string (cdr (f)) = "iris", "name is iris");
