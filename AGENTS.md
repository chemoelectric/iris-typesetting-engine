...
- **Beyond TeX Optimization**: High-performance microtypography layout engine incorporating:
  ...
  - **Structured Programming & Exception Rules**:
    - When writing code in a procedural, imperative style, always write
      only strict structured code. There shall be nothing resembling a
      goto, except for a mandatory ‘break’ at the end of every
      switch-case block in a C-like language. The only time a
      ‘case <something>:’ in such a language may end without a
      ‘break’ is if followed immediately by another
      ‘case <something>:’ An early return statement is considered
      a goto and is banned. A function in Ada or Fortran, therefore,
      must have one and only one return statement, and it must be the
      last statement. A C or C-like function that returns a value must
      have one and only one return statement, and that must be the
      last statement.
    - Raising exceptions is allowed, however, and is not subject to the
      rules of structured programming. Exceptions may not be used as
      mere methods of branching however. They must correspond to
      exceptional circumstances, such as error conditions. They may be
      used instead of returning an error code.
    - **Functional Exemption**: Functional languages (**Scheme** and
      **ATS2**) are explicitly exempt from single-exit structured
      programming rules. In imperative languages (Ada, C, Fortran,
      etc.), procedural subprograms and main programs must strictly
      follow single-entry/single-exit control flow (no early `return`
      statements). For any top-level or nested subprogram, as well as
      main programs, the modified McCabe cyclomatic complexity shall
      strictly be 10 or less.
    - **Procedural Extraction over Comments**: In all languages (both
      imperative and functional), do NOT write large monolithic
      subprograms with blocks of comments explaining subsections.
      Instead, extract sub-actions, loop bodies, and branching logic
      into small, expressive, named nested or helper subprograms,
  ...
  - **Scheme Coding Standards & Maximum Branch Metric**:
    ...
    - **Binary vs. Text I/O Rule**: `write` produces textual S-expressions. To perform binary output in R⁷RS Scheme, strictly use bytevectors with `write-bytevector`, or `write-u8` for writing an individual byte.
    - **Avoidance of Classes and Keywords**: Strictly avoid object-oriented class systems (such as Gauche's `<class>`, `define-class`, `define-method`) and keyword arguments. Prefer clean, procedural and functional R⁷RS designs with simple procedures and standard record types (`define-record-type`).
