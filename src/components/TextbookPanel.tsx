...
export const TEXTBOOK_CHAPTERS: Chapter[] = [
  {
    ...
    content: {
      ...
      sections: [
        ...
        {
          heading: '1.6 Beyond Coarse Glyph Scaling: Physical Medium Microtypography & Ink Trap Pre-Compensation',
          text: 'Legacy microtypography (e.g., the 1990s hz-program or standard TeX extensions) was strictly limited to coarse macro-adjustments—namely, slightly expanding or condensing glyph widths by 1-2% to adjust line breaks and hyphenation. The Iris Engine fundamentally redefines microtypography by extending sub-pixel vector adjustments directly to the physical characteristics of the target medium. In physical printing, acute interior join angles (such as in "M", "N", "W", "A", "v", "k", and delicate serif junctions) suffer from capillary surface tension accumulation—causing fluid ink pooling, line distortion, and counter-space clogging (ink trapping). Derived from the Master Field Equation and capillary surface potentials, Iris microtypography dynamically injects micro-notch inward offsets and physical ink-trap relief geometry into glyph vectors. Furthermore, stroke weights, stem widths, and inner counter shapes are pre-compensated for paper fiber dot gain, ink viscosity absorption rates, xerographic toner scatter, and TFT display subpixel aperture point-spread functions (PSF)—achieving true physical-medium typographic perfection.',
          equation: '\\Delta x_{\\text{InkTrap}} = -\\oint_{\\text{Corner}} \\left( \\gamma_{\\text{SurfaceTension}} \\cdot \\nabla \\phi_{\\text{Capillary}} \\right) d\\mathbf{s} \\quad \\Longrightarrow \\quad \\text{Physical Medium Microtypography}',
          notes: [
            'Rejection of Primitive Glyph Scaling: Supersedes legacy hz-program coarse font expansion with sub-pixel physical vector pre-compensation.',
            'Dynamic Physical Ink-Trap Relief: Injects calculated micro-notches into acute interior glyph join angles based on capillary surface tension and ink viscosity.',
            'Medium-Specific Counter & Stem Optimization: Pre-compensates for paper stock capillary absorption, dot gain, xerographic toner dispersion, and TFT aperture PSF.',
            'HVS Retinal Alignment: Guarantees that after physical ink absorption or display photon diffraction, the perceived optical image matches the ideal geometric design.',
          ],
        },
        {
          heading: '1.7 Spline Curvature Dynamics, Hobby Tension Mechanics & Synthetic Emboldening',
          text: 'The mathematical mechanics of synthetic emboldening (algorithmic parallel stroke expansion via normal vector offset) depend critically on the tension parameterization of glyph Bézier splines. In John Hobby’s spline model, the curvature profile κ(s) and control point length are regulated by the tension parameter τ. High-tension splines (τ > 1) concentrate curvature variations into localized inflection zones and tight control arms; under an algorithmic parallel offset r_offset(s) = r(s) + δ · n(s), high-tension outlines suffer from severe cusp formation, self-intersection loops, and stroke pinching. Conversely, low-tension splines—such as those defining Sorts Mill Goudy—exhibit exceptionally smooth, low-gradient curvature profiles across their arcs. When subjected to synthetic emboldening in digital reader engines (e.g., Google Play Books), low-tension splines absorb the parallel offset transformation without generating geometric singularities or unnatural weight imbalances, producing an unexpectedly organic and harmonious bold form purely by virtue of their underlying Hobby spline curvature distribution.',
          equation: '\\mathbf{r}_{\\text{offset}}(s) = \\mathbf{r}(s) + \\delta \\cdot \\mathbf{n}(s), \\quad \\kappa_{\\text{offset}}(s) = \\frac{\\kappa(s)}{1 - \\delta \\cdot \\kappa(s)} \\quad \\Big| \\quad \\tau_{\\text{Hobby}} \\text{ Low } \\implies |\\nabla \\kappa(s)| \\ll 1',
          notes: [
            'Hobby Spline Tension Mechanics: Demonstrates why low-tension Bézier curves maintain structural stability under parallel offset transformations.',
            'Curvature Singularity Prevention: Prevents offset singularities (1 - δ · κ(s) = 0) that cause unsightly stroke kinks and self-intersections in high-tension glyphs.',
            'Sorts Mill Goudy Empirical Behavior: Explains why Sorts Mill Goudy renders cleanly under synthetic emboldening in eBook rendering engines.',
            'Validation of Native Weight Design: Reaffirms that while low-tension splines gracefully tolerate algorithmic emboldening, optical perfection is best served by purposeful single-weight master typography.',
          ],
        },
        {
          heading: '1.8 Generalized Hobby-SVD Spline Fitting Engine & Elimination of Bound Restrictions',
          text: 'John Hobby’s classic METAFONT spline algorithm constructs smooth C²-continuous cubic Bézier curves through a sequence of knots by setting up a linear system of mock-curvature equations for tangent angles θ_i and φ_i. In original 1980s METAFONT implementations, rigid artificial bounds and specialized tridiagonal matrix assumptions were imposed on knot angles, tension parameters τ, and boundary conditions to avoid numerical ill-conditioning and division-by-zero errors. By reformulating Hobby’s system matrix M_Hobby using Singular Value Decomposition (SVD)—decomposing M = U Σ Vᵀ and computing the Moore-Penrose pseudoinverse M⁺ = V Σ⁺ Uᵀ—the spline engine directly solves singular, degenerate, or ill-conditioned linear systems. This generalized Hobby-SVD formulation completely eliminates legacy METAFONT bound restrictions, allowing the layout engine to process arbitrary generalized inputs, coincident knots, infinite-tangent slopes, and sharp corner constraints in Cl(4,1,1) space with complete mathematical stability and sub-pixel precision.',
          equation: 'M_{\\text{Hobby}} \\boldsymbol{\\theta} = \\boldsymbol{\\psi} \\quad \\Longrightarrow \\quad \\boldsymbol{\\theta} = V \\Sigma^+ U^T \\boldsymbol{\\psi} \\quad (\\text{Generalized Hobby-SVD Spline Engine})',
          notes: [
            'SVD Pseudoinverse Resolution: Resolves singular, ill-conditioned, or degenerate knot matrices using M⁺ = V Σ⁺ Uᵀ.',
            'Elimination of METAFONT Bounds: Removes artificial angular and tension constraints inherent to legacy 1980s tridiagonal METAFONT solvers.',
            'Arbitrary Constraint Processing: Enables smooth, robust curve generation across generalized inputs, sharp corner injection, and arbitrary Cl(4,1,1) spatial knot sequences.',
  ...
  },
  {
    ...
    content: {
      ...
      sections: [
        ...
        {
          heading: '7.6 Application Writer Implementation Guide: Font Disambiguation for Desktops (KDE/Plasma) & Graphics Tools (GIMP, Inkscape, Scribus)',
          text: 'To overcome Fontconfig\'s structural defect where its distance-minimization pattern matcher (`fonts-conf(5)`) selects nearest-neighbor fonts regardless of divergence from the intended pattern, application developers constructing desktop environments (KDE/Plasma) and creative applications (GIMP, Inkscape, Scribus, LibreOffice) must implement Iris Disambiguation Protocols: (1) Enforce strict candidate set evaluation rather than accepting single `FcFontMatch` returns; (2) Perform deep OpenType `name` table inspection (Name IDs 1, 2, 6, 16, 17) and `GSUB` feature parsing (`smcp`, `c2sc`, `swsh`) to differentiate files like Mokka Regular vs. Mokka Small Caps; (3) Verify byte-identity across candidate files (`FcFontSort`); (4) Serialize full PostScript names (`Name ID 6`) or qualified pattern specifiers (`Family:style=Subfamily`) in document formats (`.svg`, `.xcf`, `.sla`) and desktop theme configurations (`kdeglobals`); and (5) Provide an explicit UI/console warning when non-identical candidates match a loose pattern, prohibiting arbitrary font substitution.',
          equation: '\\text{Application } \\mathcal{A} \\quad \\implies \\quad \\text{Serialize } \\text{ID}_6 \\, (\\text{PostScript Name}) \\quad \\land \\quad \\text{Validate } \\text{ID}_{17} \\, (\\text{Typographic Subfamily}) \\quad \\land \\quad \\text{Reject Nearest-Neighbor Fallback}',
          notes: [
