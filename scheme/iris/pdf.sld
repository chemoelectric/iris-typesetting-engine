  ...
  (begin
    ...
    (define (pdf-init filename . compress-opt)
      (let* ((compress? (if (null? compress-opt) #f (car compress-opt)))
             ...
             (doc (make-pdf-doc filename port 0 0 '() 0 '() '() "" 612.0 792.0 compress? #f)))
    ...
    (define (pdf-close! pdf)
      (if (pdf-doc-read-mode? pdf)
          ...
          (begin
            ...
            (let ((cat-id (+ (pdf-doc-obj-count pdf) 1)))
              (pdf-doc-obj-count-set! pdf cat-id)
            ...
            (let ((pages-id (+ (pdf-doc-obj-count pdf) 1)))
              (pdf-doc-obj-count-set! pdf pages-id)
            ...
            (let ((font-id (+ (pdf-doc-obj-count pdf) 1)))
              (pdf-doc-obj-count-set! pdf font-id)
